...
:MoveLang
robocopy "!FilePath!GenshinImpact_Data\Persistent\AudioAssets" "!FilePath!GenshinImpact_Data\StreamingAssets\Audio\GeneratedSoundBanks\Windows" /e /copy:DAT /dcopy:DAT /move
...
for /L %%i in (1,1,4) do (
	...
	if NOT "!CurrentLanguage!"=="None" (
		for /F "usebackq delims=" %%j in ("!FilePath!Cleanup_!CurrentLanguage!_!oldVer!-!newVer!.txt") do (
			if exist "!FilePath!%%j" (
				attrib -R "!FilePath!%%j"
				del "!FilePath!%%j"
			)
...
for %%F in (
	...
	hpatchz.exe
    hdiffz.exe
    Cleanup_Common_!oldVer!-!newVer!.txt
    Cleanup_Chinese_!oldVer!-!newVer!.txt
    Cleanup_English_!oldVer!-!newVer!.txt
    Cleanup_Japanese_!oldVer!-!newVer!.txt
    Cleanup_Korean_!oldVer!-!newVer!.txt
